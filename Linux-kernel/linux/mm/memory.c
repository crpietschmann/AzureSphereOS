 ...
 * 
 * Microsoft July 2018 - Add support for CoW for XIP mapped pages
 */
		...
		struct page *old_page)
{
	...
	if (is_zero_pfn(pte_pfn(orig_pte))) {
		...
		if (old_page == NULL && fe->flags & FAULT_FLAG_REMOTE && vma->vm_ops && vma->vm_ops->access) {
			// This is a remote access with no old page so we can't try to just copy from a vaddress
			// as we're in another process.  Instead use the access hook if present.
			void *kaddr = kmap_atomic(new_page);
			vma->vm_ops->access(vma, fe->address & PAGE_MASK, kaddr, PAGE_SIZE, 0);
			kunmap_atomic(kaddr);
		} else {
...
static int do_cow_fault(struct fault_env *fe, pgoff_t pgoff)
{
	...
	// Temporary fix for non-DAX XIP FSes.  This is properly handled in 4.11 via VM_FAULT_DONE_COW
	if (fault_page && !(ret & VM_FAULT_DAX_LOCKED))
	...
	if (fault_page && !(ret & VM_FAULT_DAX_LOCKED)) {
		unlock_page(fault_page);
		put_page(fault_page);
	} else if (ret & VM_FAULT_DAX_LOCKED) {
		...
		unsigned long addr, void *buf, int len, unsigned int gup_flags)
{
	...
	while (len) {
		...
		ret = get_user_pages_remote(tsk, mm, addr, 1,
				gup_flags, &page, &vma);
		if (ret <= 0) {
			// This previously had #ifndef CONFIG_HAVE_IOREMAP_PROT but that was unnneeded
			// given access() methods that use ioremap_prot are already protected by ifdefs
			// 
			// Removing this enables XIP file systems + ptrace to work
