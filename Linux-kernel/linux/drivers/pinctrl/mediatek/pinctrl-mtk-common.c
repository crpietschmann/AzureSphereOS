 ...
 * Modifications July 2018 to support mt3620
 *
...
{
	unsigned int regmap;
	if (pctl->devdata->spec_get_regmap)
		regmap = pctl->devdata->spec_get_regmap(pin);
	else if (*pin >= pctl->devdata->type1_start && *pin < pctl->devdata->type1_end)
		regmap = 1;
	else
		regmap = 0;
	return pctl->regmap[regmap];
}
...
static unsigned int mtk_get_port(struct mtk_pinctrl *pctl, unsigned long pin)
{
	...
	return ((pin >> pctl->devdata->offset_bits) & pctl->devdata->port_mask)
			<< pctl->devdata->port_shf;
}

// For some MTK parts doing a get value (din) has a different shift
static unsigned int mtk_get_port_get(struct mtk_pinctrl *pctl, unsigned long pin)
{
	unsigned char shift = pctl->devdata->port_shf;

	if (pctl->devdata->get_port_shf)
		shift = pctl->devdata->get_port_shf;
...
static unsigned int mtk_get_bit(struct mtk_pinctrl *pctl, unsigned long pin)
{
	/* offset bits are # of unset low bits in port_mask plus the 4 that get shifted out. */
	unsigned int offset_bits = pctl->devdata->offset_bits;
			...
			bool input)
{
	...
	struct regmap *regmap = mtk_get_regmap(pctl, &offset);

	reg_addr = mtk_get_port(pctl, offset) + pctl->devdata->dir_offset;
	bit = mtk_get_bit(pctl, offset);
		...
		int value, enum pin_config_param arg)
{
	...
	unsigned int bit;
	struct regmap *regmap = mtk_get_regmap(pctl, &pin);
	 ...
	 */
	if (pctl->devdata->spec_ies_smt_set) {
		return pctl->devdata->spec_ies_smt_set(regmap,
			pin, pctl->devdata->port_align, value, arg);
	}

	bit = mtk_get_bit(pctl, pin);
...
{
	unsigned int bit;
	unsigned int reg_pullen, reg_pullsel, reg_pulldn;
	int ret;
	struct regmap *regmap = mtk_get_regmap(pctl, &pin);
	...
	if (pctl->devdata->spec_pull_set) {
		ret = pctl->devdata->spec_pull_set(regmap,
	...
	if (pctl->devdata->pullen_offset != MTK_PINCTRL_NOT_SUPPORT) {
		if (enable)
			...
			reg_pullen = CLR_ADDR(mtk_get_port(pctl, pin) +
				pctl->devdata->pullen_offset, pctl);
		regmap_write(regmap, reg_pullen, bit);
	}

	if (pctl->devdata->pullsel_offset != MTK_PINCTRL_NOT_SUPPORT) {
		if (isup)
			...
			reg_pullsel = CLR_ADDR(mtk_get_port(pctl, pin) +
				pctl->devdata->pullsel_offset, pctl);
		regmap_write(regmap, reg_pullsel, bit);
	}

	if (pctl->devdata->pulldn_offset != MTK_PINCTRL_NOT_SUPPORT) {
		if (enable && !isup)
			reg_pulldn = SET_ADDR(mtk_get_port(pctl, pin) +
				pctl->devdata->pulldn_offset, pctl);
		else
			reg_pulldn = CLR_ADDR(mtk_get_port(pctl, pin) +
				pctl->devdata->pulldn_offset, pctl);
		regmap_write(regmap, reg_pulldn, bit);
	}
...
static int mtk_pconf_pin_set(struct pinctrl_dev *pctldev, unsigned int pin,
				 unsigned long *configs, unsigned num_configs)
{
	int i, ret;

	for (i = 0; i < num_configs; i++) {
		ret = mtk_pconf_parse_conf(pctldev, pin,
			pinconf_to_config_param(configs[i]),
			pinconf_to_config_argument(configs[i]));
		if (ret < 0)
			return ret;
...
static const struct pinconf_ops mtk_pconf_ops = {
	...
	.pin_config_set	= mtk_pconf_pin_set,
};
...
static int mtk_pmx_set_mode(struct pinctrl_dev *pctldev,
		unsigned int pin, unsigned long mode)
{
	unsigned int reg_addr;
	unsigned char bit;
	unsigned int val;
	struct mtk_pinctrl *pctl = pinctrl_dev_get_drvdata(pctldev);
	unsigned int mask = (1L << pctl->devdata->mode_bits) - 1;
	struct regmap *regmap = mtk_get_regmap(pctl, &pin);
	
	if (pctl->devdata->pinmux_offset == MTK_PINCTRL_NOT_SUPPORT)
		return -EINVAL;

	if (pctl->devdata->spec_pinmux_set)
		pctl->devdata->spec_pinmux_set(regmap,
					pin, mode);

	reg_addr = ((pin / pctl->devdata->modes_per_reg) << pctl->devdata->mux_port_shf)
			+ pctl->devdata->pinmux_offset;

	mode &= mask;
	bit = pin % pctl->devdata->modes_per_reg;
	mask <<= (pctl->devdata->mode_bits * bit);
	val = (mode << (pctl->devdata->mode_bits * bit));
	return regmap_update_bits(regmap,
			reg_addr, mask, val);
}
...
static int mtk_gpio_get_direction(struct gpio_chip *chip, unsigned offset)
{
	...
	regmap_read(regmap, reg_addr, &read_val);
	return !!(read_val & bit);
}
...
static int mtk_gpio_get(struct gpio_chip *chip, unsigned offset)
{
	...
	struct regmap *regmap = mtk_get_regmap(pctl, &offset);
	unsigned int din_offset = pctl->devdata->din_offset;

	if (pctl->devdata->spec_din_offset)
		din_offset = pctl->devdata->spec_din_offset(pin);

	if (mtk_gpio_get_direction(chip, pin))
		reg_addr = mtk_get_port(pctl, offset) +
			pctl->devdata->dout_offset;
	else
		reg_addr = mtk_get_port_get(pctl, offset) +
			din_offset;
