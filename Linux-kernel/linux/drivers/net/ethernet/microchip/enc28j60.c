	...
	do {
		...
		if ((intflags & EIR_RXERIF) != 0) {
			...
			if (enc28j60_get_free_rxfifo(priv) <= 0) {
				...
				/* Ok, this is .. weird. There's buffer space, but the
				 * hardware still didn't receive the packet.
				 * If EPKTCNT=0, this indicates that the receive hardware
				 * hangs, and needs a reset. */
				if (!locked_regb_read(priv, EPKTCNT)) {
					mutex_lock(&priv->lock);
					nolock_reg_bfclr(priv, ECON1, ECON1_RXEN);
					nolock_reg_bfset(priv, ECON1, ECON1_RXRST);
					nolock_reg_bfclr(priv, ECON1, ECON1_RXRST);
