...
static void copy_to_if_dqblk(struct if_dqblk *dst, struct qc_dqblk *src)
{
	...
	if (src->d_fieldmask & QC_SPC_TIMER)
		dst->dqb_valid |= QIF_BTIME;
	if (src->d_fieldmask & QC_INO_TIMER)
		dst->dqb_valid |= QIF_ITIME;
}
...
static struct super_block *quotactl_block(const char __user *special, int cmd)
{
	...
	if (quotactl_cmd_write(cmd))
		...
		sb = get_super(bdev);
	if (!sb && MAJOR(bdev->bd_dev) == MTD_BLOCK_MAJOR) {
		sb = user_get_super(bdev->bd_dev);
	}
	bdput(bdev);
	if (!sb)
